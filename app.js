...
function generateConsistentScenarios() {
    ...
    descriptors.forEach(descriptor => {
        variants[descriptor].forEach(variant => {
            consistentScenarios.push({ descriptor, variant });
        });
    });
    let resultHtml = "<h3>Consistent Scenarios:</h3><ul>";
    consistentScenarios.forEach(scenario => {
        resultHtml += `<li>${scenario.descriptor}: ${scenario.variant}</li>`;
    });
    resultHtml += "</ul>";
    document.getElementById("consistentScenariosResult").innerHTML = resultHtml;
}
